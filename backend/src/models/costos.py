...
class Finca(UUIDPKMixin, TenantMixin, TimestampMixin, SoftDeleteMixin, AuditUserMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "codigo", name="uq_fincas_tenant_codigo"),
    )

    nombre: Mapped[str] = mapped_column(String(200), nullable=False)
    codigo: Mapped[str] = mapped_column(String(50), nullable=False)
    area_total_ha: Mapped[Decimal] = mapped_column(
...
class Lote(UUIDPKMixin, TenantMixin, TimestampMixin, SoftDeleteMixin, AuditUserMixin, Base):
    __tablename__ = "lotes"
    __table_args__ = (
        UniqueConstraint("tenant_id", "codigo", name="uq_lotes_tenant_codigo"),
    )

    finca_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("fincas.id"), nullable=False, index=True
    )
    nombre: Mapped[str] = mapped_column(String(200), nullable=False)
    codigo: Mapped[str] = mapped_column(String(50), nullable=False)
    ...
    estado: Mapped[str] = mapped_column(
        String(30), default="disponible", nullable=False
    )
    notas: Mapped[str | None] = mapped_column(Text)
    metadata_: Mapped[dict[str, Any]] = mapped_column(
        "metadata", JSONB, default=dict, nullable=False
...
class Cultivo(UUIDPKMixin, TenantMixin, TimestampMixin, SoftDeleteMixin, AuditUserMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "codigo", name="uq_cultivos_tenant_codigo"),
    )
...
class CicloProductivo(UUIDPKMixin, TenantMixin, TimestampMixin, SoftDeleteMixin, AuditUserMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "codigo", name="uq_ciclos_tenant_codigo"),
    )

    lote_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("lotes.id"), nullable=False, index=True
    )
    cultivo_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("cultivos.id"), nullable=False, index=True
    )
    codigo: Mapped[str] = mapped_column(String(50), nullable=False)
    ...
    estado: Mapped[EstadoCiclo] = mapped_column(
        SAEnum(EstadoCiclo, name="estado_ciclo", native_enum=True),
        default=EstadoCiclo.PLANIFICADO,
        nullable=False,
    )
    ...
    unidad_produccion: Mapped[str] = mapped_column(
        String(30), default="kg", nullable=False
    )
    ...
    ingreso_total: Mapped[Decimal] = mapped_column(
        Numeric(18, 4), default=Decimal("0"), nullable=False
    )
    margen_bruto: Mapped[Decimal] = mapped_column(
        Numeric(18, 4), default=Decimal("0"), nullable=False
    )
    notas: Mapped[str | None] = mapped_column(Text)
    metadata_: Mapped[dict[str, Any]] = mapped_column(
...
class Actividad(UUIDPKMixin, TenantMixin, TimestampMixin, SoftDeleteMixin, AuditUserMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "codigo", name="uq_actividades_tenant_codigo"),
    )

    nombre: Mapped[str] = mapped_column(String(200), nullable=False)
    codigo: Mapped[str] = mapped_column(String(50), nullable=False)
    tipo_costo: Mapped[TipoCosto] = mapped_column(
        SAEnum(TipoCosto, name="tipo_costo", native_enum=True), nullable=False
    )
...
class CostoActividad(UUIDPKMixin, TenantMixin, TimestampMixin, SoftDeleteMixin, AuditUserMixin, Base):
    ...
    ciclo_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("ciclos_productivos.id"), nullable=False, index=True
    )
    actividad_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("actividades.id")
    )
    lote_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("lotes.id"), nullable=False, index=True
    )
    ...
    metodo_prorrateo: Mapped[MetodoProrrateo] = mapped_column(
        SAEnum(MetodoProrrateo, name="metodo_prorrateo", native_enum=True),
        default=MetodoProrrateo.DIRECTO,
        nullable=False,
    )
...
class CostoSnapshot(UUIDPKMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("ciclo_id", "fecha", name="uq_snapshot_ciclo_fecha"),
    )

    tenant_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
    ciclo_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("ciclos_productivos.id"), nullable=False
    )
    fecha: Mapped[date] = mapped_column(Date, nullable=False)
    costo_mano_obra: Mapped[Decimal] = mapped_column(
        Numeric(18, 4), default=Decimal("0"), nullable=False
    )
    costo_insumos: Mapped[Decimal] = mapped_column(
        Numeric(18, 4), default=Decimal("0"), nullable=False
    )
    costo_maquinaria: Mapped[Decimal] = mapped_column(
        Numeric(18, 4), default=Decimal("0"), nullable=False
    )
    costo_servicios: Mapped[Decimal] = mapped_column(
        Numeric(18, 4), default=Decimal("0"), nullable=False
    )
    costo_otros: Mapped[Decimal] = mapped_column(
        Numeric(18, 4), default=Decimal("0"), nullable=False
    )
    costo_total: Mapped[Decimal] = mapped_column(
        Numeric(18, 4), default=Decimal("0"), nullable=False
    )
    costo_por_hectarea: Mapped[Decimal] = mapped_column(
