...
export default function LotesPage() {
  ...
  const fetchFincas = async () => {
    ...
    setFincas(data);
    if (data.length > 0) {
      setForm(prev => ({ ...prev, finca_id: data[0].id }));
    }
  };
  ...
  // Formato de pesos para el costo acumulado
        ...
        <div className="md:col-span-1">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button type="submit" className="w-full bg-amber-600 text-white py-2 rounded-lg font-semibold hover:bg-amber-700">+ Crear Lote</button>
            </form>
          </div>
        </div>
                      ...
                      return (
                        <tr key={lote.id} className="hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3">
                            <span className="bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs font-medium">{lote.estado}</span>
                          </td>
                          {/* === MOSTRAR COSTO ACUMULADO === */}
                          <td className="px-4 py-3 font-bold text-red-600">
                            {formatCurrency(lote.costo_acumulado || 0)}
                          </td>
                          {/* =============================== */}
                        </tr>
                      );
